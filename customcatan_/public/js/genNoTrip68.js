 ...
 * that has no three of the same resource next to each other
 * and no 6s or 8s next to each other.
 *
 */ 

function genNoTrip68() {
...
var noDubNumber = function recurse (hex, num) {    
    // if desert skip number assignment
    if (hex.resource != 'desert') {       
        ...
        // adds used number to number array
        numArray.push(n);
...
// function for checking adjacent resources - input 0
var noTripResource = function thisGuy (hex, num) { 
    ...
    for (i = 0, j = hex.adjacent.length; i < j; i++) {
        if (n == hex.adjacent[i].resource) {
            ...
            for (k = 0, l = hex.adjacent.length; k < l; k++) {
                for (m = 0, o = hex.adjacent[i].adjacent.length; m < o; m++) {
                    // console.log(hex.adjacent[i].adjacent[m]);
                    // if they have same neighbors
                    if (hex.adjacent[k] == hex.adjacent[i].adjacent[m]) {
                        // if neighbor has same resource, disqualify
                        if (hex.adjacent[k].resource == n) {
...
while (hex1.resource == null) {
    ...
    noTripResource(hex1, 0);
    noTripResource(hex2, 0);
    noTripResource(hex3, 0);
    noTripResource(hex4, 0);
    noTripResource(hex5, 0);
    noTripResource(hex6, 0);
    noTripResource(hex7, 0);
    noTripResource(hex8, 0);
    noTripResource(hex9, 0);
    noTripResource(hex10, 0);
    noTripResource(hex11, 0);
    noTripResource(hex12, 0);
    noTripResource(hex13, 0);
    noTripResource(hex14, 0);
    noTripResource(hex15, 0);
    noTripResource(hex16, 0);
    noTripResource(hex17, 0);
    noTripResource(hex18, 0);
    noTripResource(hex19, 0);
}
...
var numDraw = function (hex, xloc, yloc) {
    ...
    if (hex.resource == 'desert') {
        ...
        // wait for images to be drawn first
        context2.beginPath();
