...
//        toast(hangText + " on, " + restText + " off for " + repText + " reps");
        ...
        */
        ...
        final TextView tick = (TextView) findViewById(R.id.tick);
//        clock.setText(" ");
//
        new CountDownTimer(5000, 500) {

            public void onTick(long millisUntilFinished) {
                clock.setText("Start in");
                int temp = (Math.round(millisUntilFinished)+1000) / 1000;
                tick.setText(Integer.toString(temp));
            }
...
//
        ...
        for (int i = 0; i < repInt-1; i++) {
            // rests
            new CountDownTimer(5000 + 1000 * i * (hangInt + restInt) + restInt * 1000 + hangInt * 1000, 500) {
                Spinner rest = (Spinner) findViewById(R.id.restSpin);
                String restText = rest.getSelectedItem().toString();
                int restInt = Integer.parseInt(restText);
                ...
                public void onTick(long millisUntilFinished) {
                    ...
                    if (millisUntilFinished <= restInt * 1000) {
                        clock.setText("Rest for");
//                        tick.setText(Math.round((millisUntilFinished + 1000) / 1000));
                        int temp = (Math.round(millisUntilFinished)+1000) / 1000;
                        tick.setText(Integer.toString(temp));
        ...
        for (int j = 0; j < repInt; j++) {
            // hangs
            new CountDownTimer(5000+1000*j*(hangInt+restInt)+hangInt*1000, 500) {
                Spinner hang = (Spinner) findViewById(R.id.hangSpin);
                String hangText = hang.getSelectedItem().toString();
                int hangInt = Integer.parseInt(hangText);
                public void onTick(long millisUntilFinished) {
